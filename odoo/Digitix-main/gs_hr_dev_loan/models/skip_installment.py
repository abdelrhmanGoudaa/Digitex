...
class dev_skip_installment(models.Model):
    ...
    def get_url(self):
        for installment in self:
            ...
            action_id = self.env.ref('gs_hr_dev_loan.action_dev_skip_installment').id
            menu_id = self.env.ref('gs_hr_dev_loan.menu_skip_installment').id
            if base_url:
    ...
    def action_send_request(self):
        ...
        if self.manager_id and self.manager_id.work_email:
            ...
            template_id = self.env.ref('gs_hr_dev_loan.dev_skip_dep_manager_approval')
    ...
    def approve_skip_installment(self):
        ...
        if email:
            ir_model_data = self.env['ir.model.data']
            template_id = self.env.ref('gs_hr_dev_loan.dev_skip_ins_hr_manager_request')
            mtp = self.env['mail.template']
            template_id = mtp.browse(template_id.id)
    ...
    def dep_reject_skip_installment(self):
        if self.employee_id.work_email:
            ir_model_data = self.env['ir.model.data']
            template_id = self.env.ref('gs_hr_dev_loan.dep_manager_reject_skip_installment')
    ...
    def hr_reject_skip_installment(self):
        ...
        if self.employee_id.work_email and self.hr_manager_id:
            ir_model_data = self.env['ir.model.data']
            template_id = self.env.ref('gs_hr_dev_loan.hr_manager_reject_skip_installment')
    ...
    def confirm_skip_installment(self):
        ...
        if self.employee_id.work_email and self.hr_manager_id:
            ir_model_data = self.env['ir.model.data']
            template_id = self.env.ref('gs_hr_dev_loan.hr_manager_confirm_skip_installment')
