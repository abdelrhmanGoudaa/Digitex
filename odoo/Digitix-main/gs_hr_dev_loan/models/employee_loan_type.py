...
class employee_loan_type(models.Model):
    ...
    def _compute_loan_paid(self):
        for record in self:
            emp_loan = self.env['employee.loan'].search([('state','=','financial_approval'),('loan_type_id','=',record.id)])
    ...
    interest_type = fields.Selection([('liner','Liner'),('reduce','Reduce')],string='Interest Type',
                                     default='liner')
    #new
    payable_emp_account = fields.Many2one('account.account',string='Payable Employee Account',required="1")
    ...
    def get_action_loan_tree_done(self):
        return self._get_action('gs_hr_dev_loan.action_loan_tree_done')
    
    def get_action_loan_tree_draft(self):
        return self._get_action('gs_hr_dev_loan.action_loan_tree_draft')
    
    def action_get_hr_loan_type(self):
        return self._get_action('gs_hr_dev_loan.get_hr_loan_type')
    def get_action_loan_paid(self):
        return self._get_action('gs_hr_dev_loan.action_loan_paid')
    def get_action_hr_approval(self):
        return self._get_action('gs_hr_dev_loan.action_hr_approval')
    
    def get_loan_create(self):
        return self._get_action('gs_hr_dev_loan.action_loan_create')
    def get_all_loan(self):
        return self._get_action('gs_hr_dev_loan.action_view_all_loan')
    def get_setting(self):
        return self._get_action('gs_hr_dev_loan.action_setting')
